...
export default function FeedScreen() {
  ...
  const categoryName =
    categories.find((cat) => category && cat.value === category.value)?.label ||
    '3Design';
  ...
  const handleFollowUnfollow = async () => {
    try {
      ...
      console.log(
        `Successfully ${isFollowing ? 'unfollowed' : 'followed'} category.`
      );
    } catch (error) {
      console.error(
        `Failed to ${isFollowing ? 'unfollow' : 'follow'} category:`,
        error
      );
    }
  };
      ...
      {category && showTournamentBox && (
        <View style={styles.tournamentBox}>
          <Text style={styles.tournamentTitle}>
            Weekly Tournament for {category.label} Category
          </Text>
          {isLoadingTime ? (
